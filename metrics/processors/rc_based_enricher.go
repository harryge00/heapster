...
func (this *RcBasedEnricher) Process(batch *core.DataBatch) (*core.DataBatch, error) {
	...
	for k, v := range batch.MetricSets {
		if v.Labels[core.LabelMetricSetType.Key] == core.MetricSetTypePod {
			...
			if len(pod.OwnerReferences) == 0 || pod.OwnerReferences[0].Kind != "ReplicationController" {
				continue
			}
			rc, err := this.rcLister.ReplicationControllers(namespace).Get(pod.OwnerReferences[0].Name)
			if err != nil {
				glog.V(3).Infof("Failed to get replicationController for %s pod from cache: %v", core.PodKey(namespace, podName), err)
				continue
			}
