...
func (this *kubeletMetricsSource) decodeMetrics(c *cadvisor.ContainerInfo) (string, *MetricSet) {
	...
	if isNode(c) {
		...
		glog.V(4).Infof("decodeMetrics: ns=%s, podName=%s, cName=%s.", ns, podName, cName)
	...
	glog.V(4).Infof("decodeMetrics: metricSetKey=%s.", metricSetKey)
	...
	glog.V(1).Infof("decodeMetrics: cMetrics.MetricValues=%v.", cMetrics.MetricValues)
	...
	glog.V(4).Infof("decodeMetrics: cMetrics.LabeledMetrics=%v.", cMetrics.LabeledMetrics)
	...
	//glog.V(4).Infof("decodeMetrics: cMetrics=%v.", cMetrics)
	return metricSetKey, cMetrics
}

func (this *kubeletMetricsSource) ScrapeMetrics(start, end time.Time) *DataBatch {
	glog.V(2).Infof("ScrapeMetrics: kubeletClient=%v", this.kubeletClient)
	...
	glog.V(2).Infof("containers：%v", containers)
	...
	for _, c := range containers {
		...
		glog.V(2).Infof("name: %s, metrics: %v.", name, metrics)
