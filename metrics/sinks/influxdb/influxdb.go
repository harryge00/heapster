...
func (sink *influxdbSink) ExportData(dataBatch *core.DataBatch) {
	...
	for _, metricSet := range dataBatch.MetricSets {
		for metricName, metricValue := range metricSet.MetricValues {
			//glog.V(1).Infof("InfluxDB.ExportData: metricName=%s, metricValue=%v.", metricName, metricValue)
			var value interface{}
			...
			glog.V(6).Infof("InfluxDB.ExportData: measurementNam1=%s, Tags1=%v.", measurementName, metricSet.Labels)
		...
		for _, labeledMetric := range metricSet.LabeledMetrics {
			...
			glog.V(6).Infof("InfluxDB.ExportData: measurementName2=%s, Tags2=%v.", measurementName, point.Tags)
			dataPoints = append(dataPoints, point)
			if len(dataPoints) >= maxSendBatchSize {
				sink.sendData(dataPoints)
				dataPoints = make([]influxdb.Point, 0, 0)
