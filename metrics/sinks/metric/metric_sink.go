...
func (this *MetricSink) getAllNames(predicate func(ms *core.MetricSet) bool,
	...
	this.lock.Lock()
	defer this.lock.Unlock()
	glog.Infof("getAllNames.")

	if len(this.shortStore) == 0 {
		glog.Infof("getAllNames: shortStore's len = 0.")
		return []string{}
	}

	result := make([]string, 0, 0)
	for key, value := range this.shortStore[len(this.shortStore)-1].MetricSets {
		glog.Infof("getAllNames: key=%s, value=%v.", key, value)
		if predicate(value) {
			result = append(result, name(key, value))
		}
	}
	glog.Infof("getAllNames: result=%v.", result)
	return result
}
...
func (this *MetricSink) GetNamespaces() []string {
	return this.getAllNames(
		func(ms *core.MetricSet) bool {
			glog.Infof("GetNamespaces.predicate: key=%s, namespace=%v.", core.LabelMetricSetType.Key, core.MetricSetTypeNamespace)
			return ms.Labels[core.LabelMetricSetType.Key] == core.MetricSetTypeNamespace
		},
		func(key string, ms *core.MetricSet) string {
			glog.Infof("GetNamespaces.predicate: key=%s, labels=%v.", core.LabelNamespaceName.Key, ms.Labels[core.LabelNamespaceName.Key])
			return ms.Labels[core.LabelNamespaceName.Key]
		})
}
